...
Она уже существует и содержит рабочую Qwen Audiobook Studio.
...
- не менять `~/Desktop/Qwen Audiobook Studio.app`.

Если какие-либо из этих объектов неожиданно отсутствуют или имеют конфликтующее состояние — STOP и сообщить, ничего не восстанавливать автоматически.
...
- создавать пользовательские WAV.

Если результат не `9 tests / OK` — STOP и сообщить точную ошибку.
...
7. подтверждение: WAV не создавались;
8. подтверждение: Qwen, `.app`, книги и существующие renders не изменялись.

После этого STOP. Интеграцию Yandex в GUI выполнять отдельным следующим этапом.
