...
class NativeUIBridgeTests(unittest.TestCase):
    ...
    def test_native_mastering_and_litres_export_reuse_player_and_stay_offline(self):
        ...
        self.assertIn('Label("Формат выпуска"', source)
        self.assertIn('Button("Создать файл текущей главы")', source)
        self.assertIn('Button("Обновить файл главы")', source)
        self.assertIn('export.decision == "READY_TO_REPACKAGE"', source)
        self.assertIn('Button("Восстановить файл главы")', source)
        self.assertIn('export.decision == "READY_TO_REPAIR"', source)
        self.assertIn('Label("Мастеринг"', source)
        self.assertIn('Button("Собрать аудиокнигу")', source)
        no_selection = source.index('if delivery.selectedProfileID == nil')
        disabled_cta = source.index('Button("Собрать аудиокнигу") {}', no_selection)
        self.assertLess(disabled_cta, source.index('.disabled(true)', disabled_cta))
        self.assertIn('"--delivery-selection-status"', source)
        self.assertIn('"--set-delivery-profile"', source)
        self.assertIn('"--create-book-delivery"', source)
        self.assertNotIn('"ЛитРес"', source)
    ...
    def test_native_add_book_uses_file_importer_and_offline_bridge_only(self):
        source = (ROOT / "native" / "AudiobookStudioApp.swift").read_text(encoding="utf-8")
        contracts = (ROOT / "native" / "StudioContracts.swift").read_text(encoding="utf-8")
        add_book = swift_function_body(source, "func addBook(")
        ...
        self.assertIn('UTType(filenameExtension: "txt", conformingTo: .plainText)', source)
        self.assertIn("allowedContentTypes: [audiobookTextFileType]", source)
        self.assertIn("Поддерживается TXT в кодировке UTF-8 · до 20 МБ", source)
        self.assertIn("Загрузите всю книгу одним файлом", source)
        self.assertIn("Заголовки глав лучше размещать на отдельных строках", source)
        self.assertIn('Button("Выбрать TXT-файл")', source)
        ...
        self.assertIn("Список глав ещё не создан", source)
        self.assertIn("Перейти к подготовке текста", source)
        self.assertIn('Picker("Выберите главу", selection: $model.selectedJobID)', source)
    ...
    def test_native_book_text_preparation_has_local_gate_and_offline_bridge_only(self):
        ...
        perform = swift_function_body(source, "private func performBookTextPreparation(")
        ...
        self.assertIn("performBookTextPreparation(bookID: bookID)", confirm)
        self.assertIn('"--prepare-book-text", "--book", bookID', perform)
        self.assertIn("!result.remoteRequestSent", perform)
        self.assertNotIn("--prepare-paid-run", perform)
        self.assertNotIn("--execute-paid-plan", perform)
        self.assertNotIn("--run-", perform)
    ...
    def test_native_yandex_chapter_production_uses_separate_plan_and_confirmation(self):
        ...
        self.assertIn('provider: "yandex"', execute)
        self.assertIn("selection: plannedSelection", execute)
        self.assertNotIn("audioPath: result.outputPath", execute)
        self.assertNotIn("manifestPath: result.manifest", execute)
        ...
        self.assertIn("struct YandexChapterProgress", contracts)
        self.assertIn("struct YandexRetryApprovalResult", contracts)
        self.assertIn("Автоматический повтор запрещён", contracts)
        self.assertIn('"--yandex-chapter-progress"', source)
        ...
        self.assertIn("!result.billingChanged", source)
