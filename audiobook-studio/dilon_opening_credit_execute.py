"""Owner-gated, one-request Yandex executor for the Dilon opening credit.

The executor is deliberately downstream of the immutable PREPARE plan.  It
revalidates the exact plan, price, frozen voice and canonical Yandex endpoint
before a request.  A provider result can only become an immutable
...
_NO_REQUEST_CATEGORIES = {
    "credentials", "credentials_duplicate", "platform", "config", "input",
    "segment_limit", "pricing_gate", "pricing", "manifest",
}


class OpeningCreditExecutionError(RuntimeError):
    def __init__(self, code: str, message: str, *, provider_requests: int = 0,
                 remote_request_sent: bool = False, paid_execution: bool = False,
                 billing_changed: bool | None = False, retry_allowed: bool = False) -> None:
    ...
    try:
        if facts.sample_rate_hz == TARGET_RATE:
            ...
            result = subprocess.run([
                ...
                "-ar", str(TARGET_RATE), "-c:a", "pcm_s16le", "-fflags", "+bitexact",
                "-flags:a", "+bitexact", str(temp),
        ...
        if output.sample_rate_hz != TARGET_RATE or output.channels != 1 or output.sample_width_bytes != 2:
        ...
        if target.exists():
            ...
            if sha256_file(existing) != sha256_file(temp) or inspect_pcm_wav(existing).to_dict() != output.to_dict():
...
class OpeningCreditExternalExecutionService:
    ...
    def _plan(self, plan_id: str, plan_digest: str, today: date | None) -> dict[str, Any]:
        try:
            stored = OpeningCreditPlanStore(self.plans_root).load(
                plan_id=plan_id, expected_plan_digest=plan_digest
            )
            current = prepare_opening_credit_plan(
                pricing=self.pricing, today=today, registry_path=self.registry_path
            )
            ...
            raise OpeningCreditExecutionError(str(code), "Opening-credit PREPARE authority не подтверждена.") from error
        if (current.get("state") != "READY_FOR_OWNER_AUTHORIZATION"
    ...
    def _backend(self, plan: Mapping[str, Any]) -> None:
        try:
            ...
            raise OpeningCreditExecutionError("backend_revalidation_failed", "Yandex authority не подтверждена.") from error
        if (status.get("ok") is not True or status.get("endpoint") != DEFAULT_ENDPOINT
                ...
                or len(segments) != 1):
            raise OpeningCreditExecutionError("backend_authority_drift", "Yandex route/profile/request-cap drift.")
        segment = segments[0]
        actual = {"segment_id": segment.segment_id, "text": segment.text,
                  "pause_after_ms": segment.pause_after_ms, "paragraph_index": segment.paragraph_index}
        if actual != plan.get("segment") or make_fingerprint(segment.text, self.backend.profile) != plan.get("synthesis_fingerprint"):
            raise OpeningCreditExecutionError("backend_fingerprint_drift", "Yandex segmentation/fingerprint drift.")

    def _execution_dir(self, slug: str, job: str, plan_id: str) -> Path:
        base = review_root(workspace_root=self.workspace_root, book_slug=slug, job_id=job)
        return _safe_dir(base / "executions" / plan_id, root=self.workspace_root)

    def _authority(self, plan: Mapping[str, Any], plan_digest: str, slug: str, job: str) -> dict[str, Any]:
        routing = self.backend.request_routing_identity() if hasattr(self.backend, "request_routing_identity") else {"endpoint": DEFAULT_ENDPOINT}
        return {"schema_version": SCHEMA_VERSION, "plan_id": plan["plan_id"], "plan_digest": plan_digest,
                "book_slug": slug, "job_id": job, "text_sha256": plan["text_sha256"],
                "profile": dict(plan["profile"]), "synthesis_fingerprint": plan["synthesis_fingerprint"],
                "maximum_provider_requests": 1, "request_routing": routing}

    def _write_authority(self, path: Path, authority: Mapping[str, Any]) -> None:
        if path.is_symlink():
            raise OpeningCreditExecutionError("symlink_execution_authority", "AUTHORITY.json является symlink.")
        if path.exists():
            if _json(path, root=self.workspace_root, label="Execution AUTHORITY.json") != authority:
                raise OpeningCreditExecutionError("execution_authority_collision", "Execution authority collision.")
        else:
            atomic_write_json(path, dict(authority))

    def _entry(self, execution_dir: Path, plan: Mapping[str, Any]) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
        ...
        if not isinstance(entries, dict):
            raise OpeningCreditExecutionError("execution_manifest_mismatch", "Yandex manifest segments invalid.")
        ...
        if entry is not None and (not isinstance(entry, dict)
            ...
            raise OpeningCreditExecutionError("execution_manifest_mismatch", "Yandex segment authority mismatch.")
        return manifest, entry

    def _existing(self, execution_dir: Path, plan: Mapping[str, Any]) -> tuple[Path, int, bool] | None:
        manifest, entry = self._entry(execution_dir, plan)
        if entry is None:
            return None
        status = entry.get("status")
        if status == "AMBIGUOUS":
            raise OpeningCreditExecutionError("prior_provider_result_requires_resolution", "Prior request ambiguous; retry forbidden.")
        if status == "FAILED":
            ...
            if sent:
                raise OpeningCreditExecutionError("prior_provider_result_requires_resolution", "Prior sent request FAILED; retry forbidden.")
            return None
        if status == "IN_FLIGHT":
            return None
        if status not in {"DONE", "CACHED"} or not isinstance(manifest, dict):
            raise OpeningCreditExecutionError("execution_manifest_mismatch", "Unknown provider execution state.")
        name = manifest.get("joined_wav")
        if not isinstance(name, str) or not name:
            return None
        joined = _regular(execution_dir / name, root=self.workspace_root, label="Provider opening-credit WAV")
        remote = status == "DONE"
        if remote and not entry.get("billing_transaction_id"):
            raise OpeningCreditExecutionError("billing_evidence_missing", "Provider result lacks billing evidence.")
        return joined, (1 if remote else 0), remote

    def _completed(self, execution_dir: Path, plan: Mapping[str, Any], joined: Path) -> tuple[Path, int, bool]:
        _manifest, entry = self._entry(execution_dir, plan)
        if not isinstance(entry, dict) or entry.get("status") not in {"DONE", "CACHED"}:
            raise OpeningCreditExecutionError("provider_result_unresolved", "Provider result is not DONE/CACHED.",
                                              provider_requests=1, remote_request_sent=True,
                                              paid_execution=True, billing_changed=None)
        output = _regular(joined, root=self.workspace_root, label="Provider opening-credit WAV")
        try:
            output.relative_to(execution_dir)
        except ValueError as error:
            raise OpeningCreditExecutionError("provider_output_escape", "Provider output escaped execution package.") from error
        remote = entry.get("status") == "DONE"
        if remote and not entry.get("billing_transaction_id"):
            raise OpeningCreditExecutionError("billing_evidence_missing", "Yandex request lacks billing evidence.",
                                              provider_requests=1, remote_request_sent=True,
                                              paid_execution=True, billing_changed=None)
        return output, (1 if remote else 0), remote

    def execute_authorized(self, *, book_slug: str, job_id: str, plan_id: str, plan_digest: str,
                           owner_authorized: bool, today: date | None = None) -> dict[str, Any]:
        if owner_authorized is not True:
            raise OpeningCreditExecutionError("owner_authorization_required", "Explicit owner authorization required.")
        ...
        self._write_authority(authority_path, self._authority(plan, plan_digest, slug, job))
        with production_authority_lock(self.workspace_root, provider="yandex", book_slug=slug,
                                       job_id=job, profile_id=EXPECTED_PROFILE["profile_id"], exclusive=True):
            ...
            if existing is None:
                try:
                    joined = self.backend.run_text_job(
                        OPENING_CREDIT_TEXT, execution_dir,
                        job_id=f"dilon-opening-credit-{plan_id[:16]}", pricing=self.pricing, scope="book"
                    )
                except YandexSpeechKitError as error:
                    remote = error.category not in _NO_REQUEST_CATEGORIES
                    raise OpeningCreditExecutionError(
                        "provider_result_ambiguous" if error.category in {"network_ambiguous", "resume_ambiguous"} else "provider_execution_failed",
                        "Provider execution did not produce review-ready audio.",
                        provider_requests=1 if remote else 0, remote_request_sent=remote,
                        paid_execution=remote, billing_changed=None if remote else False,
                        retry_allowed=False,
                    ) from error
                provider_wav, historical_requests, historical_remote = self._completed(execution_dir, plan, Path(joined))
            ...
            try:
                review_wav = self.normalizer(self.workspace_root, provider_wav, execution_dir / "review-ready.wav")
                candidate = prepare_review_candidate(
                    workspace_root=self.workspace_root, book_slug=slug, job_id=job, source_wav=review_wav,
                    plan_id=plan_id, plan_digest=plan_digest, synthesis_fingerprint=plan["synthesis_fingerprint"],
                    profile=plan["profile"], provider_requests=historical_requests,
                    remote_request_sent=historical_remote, paid_execution=historical_remote,
                ...
                raise OpeningCreditExecutionError(
                    code, "Opening-credit result did not reach immutable PENDING_HUMAN_REVIEW.",
                    provider_requests=current_requests, remote_request_sent=current_remote,
                    paid_execution=current_remote, billing_changed=current_remote, retry_allowed=False,
                ) from error
        return {
            "schema_version": SCHEMA_VERSION, "state": "PENDING_HUMAN_REVIEW",
            "decision": "HUMAN_LISTENING_REQUIRED", "book_slug": slug, "job_id": job,
            "plan_id": plan_id, "plan_digest": plan_digest,
            "execution_authority_path": str(authority_path), "provider_output_path": str(provider_wav),
            "review_ready_path": str(review_wav), "candidate_id": candidate["candidate_id"],
            "candidate_digest": candidate["candidate_digest"], "candidate_path": candidate["candidate_path"],
            "synthesis_fingerprint": plan["synthesis_fingerprint"], "manual_approval_published": False,
            "provider_requests": current_requests, "remote_request_sent": current_remote,
            "paid_execution": current_remote, "billing_changed": current_remote,
            "historical_provenance": {"provider_requests": historical_requests,
                                      "remote_request_sent": historical_remote,
                                      "paid_execution": historical_remote,
                                      "billing_changed": historical_remote},
            "retry_allowed": False, "whole_book_release_ready": False, "finished_at": utc_now_iso(),
        }
